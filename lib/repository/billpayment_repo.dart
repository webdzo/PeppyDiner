...
class BillRepo {
  ...
  Future<BillpaymentModel> billDetail(String id) async {
    var response = await ApiBaseHelper()
        .getMethod("/reservations/$id/generate-bill-summary");
  ...
  Future<List<PaymentmodeModel>> modes() async {
    var response =
        await ApiBaseHelper().getMethod("/reservations/payment-modes");

    List<PaymentmodeModel> maincategoryResponse = List.from(
        json.decode(response.body).map((e) => PaymentmodeModel.fromJson(e)));
